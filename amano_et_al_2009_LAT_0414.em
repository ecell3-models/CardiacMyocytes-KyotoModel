...
@{Glycolysis = 'ON'}
# 'ON' to include Lambeth model into the Kyoto model
...
System System( / )
{
	...
	Variable Variable( Oxygen )
	{
		...
		MolarConc 0.146e-3;#amano_et_al_2009
#		MolarConc 0.24e-3;
#		MolarConc 0.0;
		Fixed 1;
	}
...
#		kfCK              @( 16.05e+6 / 200 );        #  (1/M^2/ms) #amano et al (2009)
#		kbCK              @( 9.67e-3 / 200 );         #  (1/mM/ms) #amano et al (2009)
		kfCK              @( kfCK[Glycolysis] );
		kbCK              @( kbCK[Glycolysis] );
...
#<variable name="NADH" initial_value="0.8948906751612181" units="mM" />
	Variable Variable( NADH )
        ...
        @include('Glycogenolysis.em')
        @#include('Glycogenolysis_LAT.em')
        @#include('Glycogenolysis_wo_HK_LAT.em')
        @#include('Glycogenolysis_wo_HK.em')
	@#include('GP.em')
	@include('GP.em')
	@#include('HK.em')
	@include('HK.em')
...
#               Expression "( 0.2 * LAC.MolarConc * 1000.0 )";
	        Expression "( GX.Value * 0.2 * LAC.Value /60000.0)";
        }
